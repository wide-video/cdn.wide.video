...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	float beat = texture(u_audio, vec2(BEAT_POINT, 0.0)).x;
	beat = beat * beat;
	...
	//waveform at x-coord
	float hwf = texture(u_audio, vec2(uv.x,1.0)).x;
	float vwf = texture(u_audio, vec2(uv.y,1.0)).x;
	
	#ifdef ALT_MODE
	float noise = texture(u_audio, vec2(vwf,1.0)).x;
	...
	float j = (texture(u_audio, vec2(abs(coord),0.0)).x - 0.5) * 2.0;
	...
	float w1 = texture(u_audio, vec2(0.1,0.0)).x;
	float w2 = texture(u_audio, vec2(0.2,0.0)).x;
	float w3 = texture(u_audio, vec2(0.4,0.0)).x;
	float w4 = texture(u_audio, vec2(0.8,0.0)).x;
